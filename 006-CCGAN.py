...
if __name__ == '__main__':
    ...
    dataloader = DataLoader(
        ImageDataset("./data/", transforms_x=transforms_, transforms_lr=transforms_lr),
    ...
    for epoch in range(opt.n_epochs):
        for i, batch in enumerate(dataloader):
            imgs = batch["x"]
            imgs_lr = batch["x_lr"]   # 主要是给mask后的图片提供全局的信息

            masked_imgs = apply_random_mask(imgs)

            # Adversarial ground truths
            valid = Variable(torch.FloatTensor(imgs.shape[0], *discriminator.output_shape).fill_(1.0), requires_grad=False)
            fake = Variable(torch.FloatTensor(imgs.shape[0], *discriminator.output_shape).fill_(0.0), requires_grad=False)
            ...
            gen_imgs = generator(masked_imgs, imgs_lr)   # 使用mask后的图片和真实的图片去生成图片
