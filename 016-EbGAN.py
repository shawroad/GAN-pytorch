...
class Generator(nn.Module):
    '''
    生成模型: 将隐向量直接通过全连接整成我们想要的维度,然后在通过上采样生成最终的图片
    '''
    def __init__(self):
...
class Discriminator(nn.Module):
    '''
    判别模型: 先将图片进行下采样 然后通过压缩得到embedding,然后在进行上采样 得到最终的图像(类似于自编码器)
    '''
    def __init__(self):
...
if __name__ == '__main__':
    ...
    txt = DataTxt()
    dataloader = DataLoader(txt, shuffle=True, batch_size=opt.batch_size)
    ...
    for epoch in range(opt.n_epochs):
        for i, (imgs, _) in enumerate(dataloader):
            ...
            # Generate a batch of images 使用噪声去生成批量的图片
            gen_imgs = generator(z)
            recon_imgs, img_embeddings = discriminator(gen_imgs)  # 重构后的图片 和　隐层信息
